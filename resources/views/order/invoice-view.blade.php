@once
    <style>
        ...
        .invoice-page-sheet .items tbody tr.product-row td,
        .invoice-page-sheet .items tbody tr.blank-product-row td {
            height: 18px;
            line-height: 18px;
        }
        ...
        .invoice-page-sheet table.image-table {
            border-collapse: collapse;
            width: 100%;
        }

        .invoice-page-sheet .image-table td {
            border: none;
            padding: 0;
...
@foreach ($orderProductPages as $pageIndex => $pageProducts)
    ...
    <div class="sheet invoice-page-sheet {{ !$isLastInvoicePage ? 'invoice-page-break' : '' }}">
                            ...
                            <table class="image-table">
                                <tr>
                                    <td style="padding:2px 5px; border:none;">
                                        <div class="bold">
                                            {{ $listing_detail?->platform_sku ?? ($product_detail['sku_code'] ?? '') }} - {{ $listing_detail?->listing_title ?? ($product_detail['name'] ?? '') }}
                                        </div>
                                        @if($listing_detail)
                                            <div class="small text-muted">{{ ucfirst($listing_detail->platform ?? '') }} listing</div>
                                        @endif
