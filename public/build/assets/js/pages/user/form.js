        ...
        $.ajax({
            ...
            error: function (xhr) {
                ...
                if (response.whatsapp_status) {
                    const shouldRedirect = response.should_redirect_to_qr && response.redirect;
                    NotifyAlert(shouldRedirect ? 'warning' : 'error', response, response.message, shouldRedirect ? response.redirect : null);
...
$(document).on('submit', '.ajaxform_reset_form', function (e) {
    ...
    let $submitBtn = $this.find('.submit-button');
    let $oldSubmitBtn = $submitBtn.html();
    let preserveFields = String($this.data('preserve-fields') || '')
        .split(',')
    ...
    if ($ajaxform_reset_form.valid()) {
        $.ajax({
            ...
            success: function (res) {
                ...
                let preservedValues = {};
                preserveFields.forEach(function(fieldName) {
                    let $field = $this.find(`[name="${fieldName}"]`);
                    if ($field.length) {
                        preservedValues[fieldName] = $field.val();
                    }
                });
                ...
                preserveFields.forEach(function(fieldName) {
                    let $field = $this.find(`[name="${fieldName}"]`);
                    if ($field.length && Object.prototype.hasOwnProperty.call(preservedValues, fieldName)) {
                        $field.val(preservedValues[fieldName]);
                    }
                });

                NotifyAlert('success', res);
                $this.trigger('ajaxform:success', [res]);
            },
            error: function (xhr) {
                ...
                if (resp ) { //&& resp.status == 400 && resp?.redirect
                    console.log("yes in");
