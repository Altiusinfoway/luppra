...
function enableConfirmationOn(eventName, className, message, callback) {
    ...
    $(document).on("focusin", "." + className, function () {
    ...
    $(document).on(eventName, "." + className, function () {
        ...
        data = $this.data("data");
        previousValue = currentElement.dataset.previousValue ?? previousValue;
    ...
    $("#confirmationModal")
        ...
        .on("hidden.bs.modal.confirmation", function () {
            ...
            if (!isConfirmed) {
                const rollbackValue = currentElement.dataset.previousValue ?? previousValue;
                if (rollbackValue !== null && rollbackValue !== undefined) {
                    currentElement.value = rollbackValue;
                }
            } else {
                currentElement.dataset.previousValue = currentElement.value;
            }
...
function doConfirmation($this, message, callback) {
    ...
    $('#confirm-yes').off('click.doConfirmation').on('click.doConfirmation', function () {
        isConfirmed = true;
        isResolved = true;
        callback(true, $this);
        $("#confirmationModal").modal('hide');
    ...
    $('#confirmation-close').off('click.doConfirmation').on('click.doConfirmation', function () {
        isResolved = true;
        callback(false, $this);
        $("#confirmationModal").modal('hide');
    });

    $('#confirmationModal').off('hidden.bs.modal.doConfirmation').on('hidden.bs.modal.doConfirmation', function () {
        if (!isConfirmed && !isResolved && typeof callback === 'function') {
            callback(false, $this);
        }
    });
...
async function _check_address(_address) {

    return new Promise(resolve => {
        setTimeout(() => {

            const billingSelected = document.querySelector('input[name="billing_address"]:checked');
            const shippingSelected = document.querySelector('input[name="shipping_address"]:checked');

            if (billingSelected) {
                _address.billing_address = billingSelected.value;
            }

            if (shippingSelected) {
                _address.shipping_address = shippingSelected.value;
            }
