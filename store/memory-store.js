...
class ConcurrentStore extends EventEmitter {
    ...
    getLastMessage(jid) {
        ...
        return msgsArray.sort((a, b) =>
            this.normalizeTimestamp(b?.messageTimestamp) - this.normalizeTimestamp(a?.messageTimestamp)
        )[0];
    }
    ...
    async loadMessages(jid, messageId = null, options = {}) {
        ...
        if (before) {
            msgsArray = msgsArray.filter(msg =>
                this.normalizeTimestamp(msg?.messageTimestamp) < this.normalizeTimestamp(before)
            );
        }

        if (after) {
            msgsArray = msgsArray.filter(msg =>
                this.normalizeTimestamp(msg?.messageTimestamp) > this.normalizeTimestamp(after)
            );
        }

        msgsArray.sort((a, b) => {
            const dateA = this.normalizeTimestamp(a?.messageTimestamp);
            const dateB = this.normalizeTimestamp(b?.messageTimestamp);
